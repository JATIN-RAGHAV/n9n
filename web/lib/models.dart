...
Object? parseConfigValue(String text) {
  try {
...
List<Json> asJsonList(Object? value) =>
    value is List ? value.map(asJson).toList() : <Json>[];
...
class WorkflowEdge {
  ...
  factory WorkflowEdge.fromJson(Json data) => WorkflowEdge(
        ...
        source: '${data['source'] ?? ''}',
        target: '${data['target'] ?? ''}',
        sourcePort: '${data['source_port'] ?? 'out'}',
      );
  Json toJson() =>
      {'id': id, 'source': source, 'target': target, 'source_port': sourcePort};
}
...
class WorkflowDraft {
  ...
  WorkflowDraft copy() =>
      WorkflowDraft.fromJson(jsonDecode(jsonEncode(toJson())) as Json);
}
...
String? validateConnection(
    WorkflowDraft draft, String source, String target, String port) {
  ...
  if (triggerTypes.contains(destination.type)) {
    return 'Triggers cannot have incoming connections.';
  }
  if (draft.edges.any((e) => e.target == target)) {
    return 'Each action accepts one incoming connection.';
  }
  if (draft.edges.any((e) =>
      e.source == source && e.target == target && e.sourcePort == port)) {
