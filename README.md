...
Use **Test draft** to validate and run the saved graph without publishing or changing the live version. Enter sample input, then inspect each recorded node's input, output, branch result, skipped state, and failure in **Step by step**. Test runs execute configured HTTP and Gmail actions, so those nodes can affect external services.
