...
The mobile workflow editor supports the initial trigger, transform, HTTP, condition, and Gmail nodes; node configuration, graph connections, saving, publishing, activation, and manual runs. Runs and credentials can be inspected in their tabs. Gmail OAuth browser authorization remains in the web app; the mobile app can manage existing credentials or enter a refresh token manually. It does not claim to provide native Google OAuth.
