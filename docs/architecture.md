...
For production, place HTTPS in front of web, set `COOKIE_SECURE=true`, set `ALLOWED_ORIGIN` to the exact public origin, generate secrets using `make setup` before first use, and back up the volume. For local HTTP development, `COOKIE_SECURE=false` is required for login cookies. The backend allows 20 registration/login attempts per source IP per minute and at most four concurrent password checks. It records schema version 1 in SQLite and adopts the existing initial schema without deleting data. Terminal runs and their steps are retained for `RUN_RETENTION_DAYS` (30 by default); expired sessions and OAuth states are removed hourly. Webhook event IDs remain as deduplication tombstones. A replay after its run is pruned returns `{"run":null,"duplicate":true}`.

Gmail credential secrets are encrypted by `ENCRYPTION_KEY`. Workflow configuration such as webhook secrets and HTTP headers, along with run inputs and outputs, remain plaintext in SQLite; protect the volume and its backups accordingly.
